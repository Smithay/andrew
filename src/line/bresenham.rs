...
impl BresenhamLine {
    ...
    fn plot(pt1: (usize, usize), pt2: (usize, usize)) -> Vec<(usize, usize)> {
        ...
        let (delta_x, delta_y) = (pt2.0 - pt1.0, max(pt1.1, pt2.1) - min(pt1.1, pt2.1));
        // Check which octant the line is in
        if delta_x >= delta_y {
            // Octant 1 & 2
            let y_inc = delta_y as f32 / delta_x as f32;
            let mut y = pt1.1 as f32;
            for x in pt1.0..pt2.0 + 1 {
            ...
            if pt2.1 > pt1.1 {
                for y in min(pt1.1, pt2.1)..max(pt1.1, pt2.1) + 1 {
                    pts.push((x.round() as usize, y));
                    x += x_inc;
                }
                pts.push((x.round() as usize, pt1.1));
            } else {
                for y in (min(pt1.1, pt2.1)..max(pt1.1, pt2.1) + 1).rev() {
                    pts.push((x.round() as usize, y));
                    x += x_inc;
                }
            }
